...
take_screenshot() {
	...
	# Schritt 1: Screenshot NUR speichern (nicht kopieren), um ihn bearbeiten zu können
	if "$LIB_DIR/hyde/grimblast" "${extra_args[@]}" save "$mode" "$temp_screenshot"; then
		# Schritt 2: HDR→SDR Tone Mapping auf die gespeicherte Datei anwenden
		magick "$temp_screenshot" -sigmoidal-contrast 20,45% "$temp_screenshot"

		# Schritt 3: Das korrigierte Bild in die Zwischenablage kopieren
